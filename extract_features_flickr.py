...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        # Create hdf5 files to store features and boxes.
        features_f = h5py.File(
            os.path.join(args.output_dir,'features.hdf5'),
            'w')
        boxes_f = h5py.File(
            os.path.join(args.output_dir,'boxes.hdf5'),
            'w')
        scores_f = h5py.File(
            os.path.join(args.output_dir,'scores.hdf5'),
            'w')
        min_boxes = 100000
        max_boxes = 0
        mean_boxes = 0
        ...
        for im_file in tqdm(imglist):
            ...
            scores, keep_boxes = zip(*sorted(
                zip(max_conf[keep_boxes],keep_boxes),
                reverse=True,
                key=lambda x: x[0]))
            scores = list(scores)
            keep_boxes = list(keep_boxes)
            ...
            # Compute stats
            num_boxes = image_bboxes.shape[0]
            min_boxes = min(min_boxes,num_boxes)
            max_boxes = max(max_boxes,num_boxes)
            mean_boxes += num_boxes
            num_images += 1
            ...
            if num_images%500==0:
                ...
                print('Min:',min_boxes)
                print('Max:',max_boxes)
                print('Mean:',mean_boxes / num_images) 

            im_id = im_file.split('.')[0]
            features_f.create_dataset(im_id,data=image_feat)
            boxes_f.create_dataset(im_id,data=image_bboxes)
            scores_f.create_dataset(im_id,data=scores)
